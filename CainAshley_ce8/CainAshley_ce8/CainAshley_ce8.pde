...
void setup() {
  ...
  beatDetector.addMessageListener(
    new Bead() {
      protected void messageReceived(Bead b) { brightness = 1.0; }
    }
  );
...
void resynthButton() {
  ...
  resynthOn = !resynthOn;
  if(resynthOn){
    mainGain.setGain(0.075);
    waveGain.setGain(resynthVal);
  } else {
    mainGain.setGain(0.5);
    waveGain.setGain(0);
  }
}
